---
product: dd360-ds
title: Filter Select component
components: Filter Select
---

<br />
<br />

# <span name="floating-nav">Filter Select</span>

The FilterSelect component is used to create an interactive dropdown menu that allows the user to select an option from several predefined options in the application.

<br />
<br />
## <span name="floating-nav">Imports</span>

<WindowEditor codeString="import { FilterSelect } from 'dd360-ds'" />

<br />
## <span name="floating-nav">Usage</span>

Use the FilterSelect component as shown below:

<ContainerComponentDoc className="gap-y-2">
 <FilterSelectCustom />
</ContainerComponentDoc>

The code snippet below shows how to use the FilterSelect component:

<WindowEditor
    codeString={`import { FilterSelect } from 'dd360-ds'
  
<FilterSelect
    position={{
      top: 0,
      left: 0,
      show: true
    }}
    listItems={{
      A: {
        label: 'Option A'
      },
      B: {
        label: 'Option B'
      }
    }}
    onApply={handleChange}
/>
`}
/>

<br />

## <span name="floating-nav">List Items</span>

The prop <span className='border border-gray-300 bg-gray-200 p-1'>listItems</span> It is used to show the available options, there is also the possibility of disabling one of them
<ContainerComponentDoc className="gap-y-2">
 <FilterSelectCustom disabled />
</ContainerComponentDoc>


<WindowEditor
    codeString={`import { FilterSelect } from 'dd360-ds'
  
<FilterSelect
    position={{
      top: 0,
      left: 0,
      show: true
    }}
    listItems={{
      A: {
        label: 'Option A'
      },
      B: {
        label: 'Option B',
        disabled:true
      }
    }}
    onApply={handleChange}
/>
`}
/>

<br />

#### <span name="floating-nav">Position</span>

To manage the FilterSelect component's visibility and position, you can use the useState hook in a functional component. Position stores the top and left coordinates, as well as a boolean indicating whether the component should be shown. The component listens for clicks on a button, which toggles the FilterSelect visibility and updates its position based on the click event's coordinates.

<ContainerComponentDoc>
  <FilterSelectCustom />
</ContainerComponentDoc>

The code snippet below shows how to handle the visibility and position:

<WindowEditor
    codeString={`import React, { useEffect, useState } from 'react'
import { FilterSelect, Flex, Text } from 'dd360-ds'
import { FilterIcon } from '@heroicons/react/outline'

const FilterSelectCustom = () => {
 const [domLoaded, setDomLoaded] = useState<boolean>(false)
 const [position, setPosition] = useState<{
    top: number
    left: number
    show: boolean
  }>({
    top: 0,
    left: 0,
    show: false
  })

  useEffect(() => { setDomLoaded(true)}, [])

  return (
    <>
      {domLoaded && (
        <>
          <div className="h-8">
            <button  onClick={(event) => {
              setPosition((position) => ({
                top: event.pageY,
                left: event.pageX,
                show: !position.show
              }))
            >
              <Flex gap="3" justifyContent="center" alignItems="center">
                <Text className="mt-0.5 mr-1" bold>
                  FilterSelect
                </Text>
                <FilterIcon className="w-4 h-4" />
              </Flex>
            </button>
          </div>
          <FilterSelect
            position={position}
            listItems={{
              A: {
                label: 'Option A'
              },
              B: {
                label: 'Option B',
                disabled
              }
            }}
            onApply={()=>alert('handleChange)}
          />
        </>
      )}
    </>
  )
}

export default FilterSelectCustom
 `}/>

Note: The "domLoaded" state variable in the FilterSelectCustom is used to ensure that the component is fully rendered before displaying the FilterSelect component. This is important when using Next.js, as it performs server-side rendering by default. By waiting for the client-side rendering to complete, we can avoid any issues with the FilterSelect component not being fully initialized.

<br/>

## <span name="floating-nav">Title</span>

The prop <span className='border border-gray-300 bg-gray-200 p-1'>title</span> serves to provide a descriptive title or label

<ContainerComponentDoc className="gap-y-2">
 <FilterSelectCustom title="Title Example" />
</ContainerComponentDoc>


<WindowEditor
    codeString={`import { FilterSelect } from 'dd360-ds'
  
<FilterSelect
    title="Title Example"
    position={{
      top: 0,
      left: 0,
      show: true
    }}
    listItems={{
      A: {
        label: 'Option A'
      },
      B: {
        label: 'Option B'
      }
    }}
    onApply={handleChange}
/>
`}
/>

<br />

## <span name="floating-nav">Selected Value</span>

The prop <span className='border border-gray-300 bg-gray-200 p-1'>selectedValue</span> is used to provide a default value

<ContainerComponentDoc className="gap-y-2">
 <FilterSelectCustom selectedValue="A"/>
</ContainerComponentDoc>


<WindowEditor
    codeString={`import { FilterSelect } from 'dd360-ds'
  
<FilterSelect
    selectedValue="A"
    position={{
      top: 0,
      left: 0,
      show: true
    }}
    listItems={{
      A: {
        label: 'Option A'
      },
      B: {
        label: 'Option B'
      }
    }}
    onApply={handleChange}
/>
`}
/>

<br />

## <span name="floating-nav">Text Apply Btn</span>

The prop <span className='border border-gray-300 bg-gray-200 p-1'>textApplyBtn</span> it is used to customize the text of the apply button

<ContainerComponentDoc className="gap-y-2">
 <FilterSelectCustom textApplyBtn="Apply Example"/>
</ContainerComponentDoc>


<WindowEditor
    codeString={`import { FilterSelect } from 'dd360-ds'
  
<FilterSelect
    textApplyBtn="Apply Example"
    position={{
      top: 0,
      left: 0,
      show: true
    }}
    listItems={{
      A: {
        label: 'Option A'
      },
      B: {
        label: 'Option B'
      }
    }}
    onApply={handleChange}
/>
`}
/>

<br />

## <span name="floating-nav">Text Reset Btn</span>

The prop <span className='border border-gray-300 bg-gray-200 p-1'>textResetBtn</span> it is used to customize the text of the reset button

<ContainerComponentDoc className="gap-y-2">
 <FilterSelectCustom textResetBtn="Reset Example"/>
</ContainerComponentDoc>


<WindowEditor
    codeString={`import { FilterSelect } from 'dd360-ds'
  
<FilterSelect
    textResetBtn="Reset Example"
    position={{
      top: 0,
      left: 0,
      show: true
    }}
    listItems={{
      A: {
        label: 'Option A'
      },
      B: {
        label: 'Option B'
      }
    }}
    onApply={handleChange}
/>
`}
/>

<br />

## <span name="floating-nav">Width</span>

The prop <span className='border border-gray-300 bg-gray-200 p-1'>width</span> used to customize the size

<ContainerComponentDoc className="gap-y-2">
 <FilterSelectCustom width={250}/>
</ContainerComponentDoc>


<WindowEditor
    codeString={`import { FilterSelect } from 'dd360-ds'
  
<FilterSelect
    width={250}
    position={{
      top: 0,
      left: 0,
      show: true
    }}
    listItems={{
      A: {
        label: 'Option A'
      },
      B: {
        label: 'Option B'
      }
    }}
    onApply={handleChange}
/>
`}
/>

<br />
<br />

### <span name="floating-nav">API Reference</span>

<CustomTableDocs
    dataTable={[
        { title: 'title', types: ['string'] },
		    { title: 'listItems', types: ['Object'] ,required:true},
        { title: 'selectedValue', types: ['string'] },
        { title: 'textApplyBtn', types: ['string'] },
        { title: 'textResetBtn', types: ['string'] },
        { title: 'position', types: ['Object'] },
        { title: 'className', types: ['string'] },
        { title: 'width', types: ['Object'] },
	    { title: 'onApply', default: 'null', types: ['function'] ,required:true}
    ]}

/>

Note: This documentation assumes that the audience has basic knowledge of React and how to use components in React applications.
