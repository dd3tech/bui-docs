 ...
 */
export const useIntersectionObserver = ({ targetSearchElement, searchBy, observerOptions = { rootTag: null, offset: "0px 0px 0px 0px" } }: UseIntersectionObserverProps) => {
  const [entries, setEntries] = useState<IntersectionObserverEntry[]>([]);
  const observerRef = useRef<IntersectionObserver | null>(null);

  const intersectIndex = useMemo(() => entries?.findIndex((entry) => entry?.isIntersecting), [entries])
  const newRoot = useMemo(() => observerOptions?.rootTag && document?.querySelector(observerOptions.rootTag),[observerOptions?.rootTag])

  useEffect(() => {
    const elements = getElements(targetSearchElement, searchBy)
    if (!elements?.length) return

    observerRef.current = new IntersectionObserver((newEntries) => setEntries(newEntries), {
      root: newRoot || null,
      rootMargin: observerOptions?.offset,
    });

    elements.forEach((element: HTMLElement) => observerRef.current?.observe(element));
