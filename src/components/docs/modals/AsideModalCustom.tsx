...
const variantsAsideModal = ['a', 'label', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'small', 'span', 'currency', 'anchorSmall'] as const
type TVariantAsideModal = (typeof variantsAsideModal)[number]

const AsideModalCustom = ({ propsCustom, positionAsideModal, haveVariants }: ModalCustomProps) => {
    const [activeModal, setActiveModal] = useState<boolean>(false)

    const [propCustomValue, setPropCustomValue] = useState<any>()

    const [variantAsideModal, setVariantAsideModal] = useState<TVariantAsideModal>('h4')
                ...
                {haveVariants && (
                    <div className="flex flex-wrap gap-5 mt-6 items-center">
                        {variantsAsideModal?.map((element: TVariantAsideModal) => (
                            <div className={composeClasses(activeModal && 'w-full', 'flex gap-2')} key={element}>
                                <input
